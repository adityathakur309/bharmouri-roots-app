...
export function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  const renderSidebar = ({ mobile = false }: { mobile?: boolean } = {}) => (
    <div className="flex flex-col h-full min-h-0">
      {/* Brand */}
      <div className={cn("flex items-center gap-3 p-5 border-b shrink-0", collapsed && !mobile && "justify-center px-3")}>
      ...
      {/* User */}
      <div className={cn("p-3 border-t space-y-1 shrink-0", collapsed && !mobile && "px-2")}>
  ...
  return (
    <div className="h-[100dvh] max-h-[100dvh] overflow-hidden bg-[hsl(var(--muted))]/20 flex">
      {/* Desktop sidebar — fixed height, does not scroll with page */}
      <aside
        className={cn(
          "relative hidden lg:flex flex-col bg-[hsl(var(--card))] border-r transition-all duration-300 shrink-0 h-full overflow-hidden",
          collapsed ? "w-16" : "w-64"
        )}
      >
        {renderSidebar()}
